...
class AdminWindow(BoxLayout):
    def __init__(self, **kwargs):
        ...
        prod_stable = DataTable(table=products)
        product_scrn.add_widget(prod_stable)

    def on_kv_post(self, base_widget):
        # Build dropdown in Python — fully detached from layout
        self.ids.admin_logo.source = GURU_JPG
        self.file_dropdown = DropDown(auto_dismiss=True)
        
        btn_logout = Button(text='Logout', size_hint_y=None, height=36,
                            background_normal='', background_color=(0.14, 0.42, 0.72, 1),
                            color=(1,1,1,1), font_size='14sp')
        btn_logout.bind(on_release=lambda x: (self.logout(), self.file_dropdown.dismiss()))
        
        btn_quit = Button(text='Quit', size_hint_y=None, height=36,
                        background_normal='', background_color=(0.55, 0.15, 0.15, 1),
                        color=(1,1,1,1), font_size='14sp')
        from kivy.core.window import Window
        btn_quit.bind(on_release=lambda x: Window.close())
        
        self.file_dropdown.add_widget(btn_logout)
        self.file_dropdown.add_widget(btn_quit)
        self.ids.file_trigger.bind(on_release=self.file_dropdown.open)
    ...
    def add_product(self, code, name, weight, stock, sold, order, purchase):
        ...
        if code == '' or name == '' or weight == '' or stock == '' or order == '':
            ...
            self.products.insert_one(
                {'product_code': code, 'product_name': name, 'product_weight': weight, 'in_stock': stock, 'sold': sold,
    ...
    def remove_user(self, user):
        if user == '':
            ...
            if target_user == None:
                ...
                content = self.ids.scrn_contents
                content.clear_widgets()
                self.users.delete_many({'user_name': user})
